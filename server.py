...
[검증 원칙 - 중요!]
1. **오탈자 비교 중심**: 법령보다 Standard와 Design 간의 오탈자/오기 비교를 우선시하세요.
2. **함량 정보(%) 허용**: Standard에 없어도 Design에 함량 정보(%)가 추가된 것은 문제로 보지 않습니다. 
   예: Standard에 "당근(국내산)"만 있고 Design에 "당근(국내산) 4.1%"가 있어도 허용합니다.
3. **비정상 값만 문제**: 함량 정보가 100%를 초과하거나 말도 안되는 값(예: 221%, 999%)인 경우만 문제로 표시합니다.
4. **실제 오류만 검출**: 
   - 원재료명 오기 (예: "당근"이 "당근"으로 잘못 표기)
   - 원산지 오기 (예: "국내산"이 "수입산"으로 잘못 표기)
   - 순서 위반 (배합비 순서와 다름)
   - 필수 항목 누락 (알레르기 문구 등)

[검증 항목]
1. **순서 위반**: 원재료 나열 순서가 Standard(배합비 순)와 다른지 확인.
2. **내용 누락/오기**: 
    - Standard에 있는 알레르기 문구가 Design에 있는가?
    - 원재료명이 정확히 일치하는가? (오탈자 확인)
    - 원재료의 상세 내역(괄호 안 내용)이 일치하는가? (원산지 등)
    - 함량 정보(%)는 추가되어도 문제 없음. 단, 100% 초과나 비정상 값만 문제.
3. **법률 위반**: 법령에 명시된 의무사항이 누락되었거나 잘못 표기되었는지 확인.
4. **오탈자**: 단순 글자 오류 확인.
    ...
    "issues": [
        {
            ...
            "location": "위치 (예: 원재료명 3번째 줄)",
            "issue": "오류 내용 (예: '양파'가 '당근'보다 앞에 와야 함)",
            "expected": "정답 내용",
            "actual": "실제 내용",
            "suggestion": "수정 제안",
            "law_reference": "관련 법령 조항 (법률 위반인 경우)"
        }
    ]
}
"""
