...
# 텍스트/비전 모델 이름 (원하면 .env에서 덮어쓰기 가능)
...
def call_openai_response(model: str, input_data, *, response_format=None, max_retries: int = 3):
    ...
    - input_data: 문자열 또는 Responses input(list[dict])
    """
    last_err = None
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 필요 시 timeout 인자도 추가 가능: client.responses.create(..., timeout=600)
...
def extract_output_text_from_response(response) -> str:
    ...
    - response_format={"type": "json_object"} 사용 시 JSON 문자열이 나옴
    """
    try:
...
def combine_parts_to_prompt(parts) -> str:
    """
    Gemini 스타일 parts 리스트를 텍스트 하나로 합쳐서 ChatGPT에 넘기기
    - 문자열이면 그대로
    - {'text': '...'}면 text만 추출
    """
    chunks = []
...
[추출해야 할 정보]
...
(네가 원래 쓰던 전체 프롬프트 그대로 붙여넣기)
"""

PROMPT_CREATE_STANDARD = """
당신은 식품 규정 및 표시사항 전문가입니다.
...
(원래 코드에 있던 긴 프롬프트 그대로)
"""
...
# --- OCR 함수 (OpenAI Vision + pytesseract 폴백) ---

def ocr_image_bytes(image_bytes: bytes) -> str:
    """
    이미지에서 텍스트를 추출하는 OCR 함수
    1순위: OpenAI Vision (한국어 + 숫자 + 기호 그대로)
    2순위: pytesseract (설치되어 있을 경우)
    """
    # 1) OpenAI Vision 시도
    try:
...
# --- 파일 처리 (이미지는 bytes, Excel은 텍스트) ---

def process_file_to_part(file_storage):
    """
    파일을 모델에 줄 수 있는 텍스트/바이너리 형태로 변환
    - Excel: CSV 텍스트
    - 이미지: bytes (OCR 등에서 사용)
    - 기타: 간단한 설명 텍스트
    """
    ...
    if mime_type.startswith("image/"):
        try:
            # 여기서는 리사이즈 없이 raw bytes를 넘기고, OCR 쪽에서 필요 시 리사이즈
            return {"mime_type": mime_type, "data": file_data}
        except Exception as e:
            print(f"⚠️ 이미지 처리 실패: {e}")
            return {"mime_type": mime_type, "data": file_data}

    # PDF 등 기타는 간단한 설명 텍스트로
    return {
...
def extract_ingredient_info_from_image(image_file):
    ...
    1순위: OpenAI Vision + JSON 응답
    2순위: OCR 텍스트만 반환 ({"ocr_fallback_text": ...})
    """
    try:
        ...
        input_items = [
            {
                "role": "user",
                "content": [
                    {"type": "input_text", "text": PROMPT_EXTRACT_INGREDIENT_INFO.strip()},
                    {"type": "input_image", "image_url": {"url": data_url}},
                ],
            }
        ]
...
# --- 여기서부터 simple_generate_highlight_html, extract_text_from_design_part 등 기존 함수 유지 ---

def simple_generate_highlight_html(ocr_text: str, standard_ingredients: list[str]) -> str:
    """
    (기존 코드에서 사용하던 하이라이트 HTML 생성 함수)
    """
...
def verify_design():
    ...
    # 모델이 하이라이트 안 주면 서버 폴백 하이라이트 생성
    if not highlight_html:
        print("모델에서 하이라이트를 제공하지 않음 -> 서버 폴백 OCR 하이라이트 생성")
        try:
            std_ingredients = []
            try:
